...
if __name__ == '__main__':
    ...
    if hparams.resume_from_checkpoint:
        trainer = pl.Trainer.from_argparse_args(hparams, logger=tb_logger, gpus=-1, val_check_interval=hparams.val_check_interval,
                                                ...
                                                max_epochs=max_epochs, precision=16, log_gpu_memory='all',
                                                callbacks=[lr_monitor], accelerator = 'ddp')
    else :
        trainer = pl.Trainer.from_argparse_args(hparams, logger=tb_logger,gpus=-1,val_check_interval=hparams.val_check_interval,
                                                gradient_clip_val=hparams.gradient_clip_val, log_gpu_memory='all',
